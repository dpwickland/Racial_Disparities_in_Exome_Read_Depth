...
GENE_LIST <- c('TP53','PIK3CA','MUC16','USH2A','TTN')
  ...
  #all_bams_all_mutations_master$race <- factor(all_bams_all_mutations_master$race, levels = c("White","Black")) 
  ...
  pdf(paste0("/home/mayo/m187735/s212975.Wickland_Immunomics/processing/TCGA/plots/",GENE,"_depth_from_VCF_by_race_BAM.pdf"),height=14,width=13)
  ...
  ###############################################
  #3.5 GENE READ-DEPTH BY RACE (calculated from vcf) FOR PATIENTS *WITH* MUTATIONS IN THAT GENE IN VCF
  ############################################### 
  
  pdf(paste0("/home/mayo/m187735/s212975.Wickland_Immunomics/processing/TCGA/plots/",GENE,"_depth_from_VCF_by_race_VCF.pdf"),height=14,width=13)
  
  #function for calculating n
  n_fun <- function(x){return(data.frame(y=-1.75,label=paste(length(x),"mutation-\npatient pairs")))} 
  
  #first, melt the dataframE
  from_VCF_melted <- cdriver_master[,c("submitter_id","chromosome","mutation_position","race","Cancer","AD_total_VCF","AD_ref_VCF","AD_alt_VCF")]
  from_VCF_melted <- melt(from_VCF_melted,id.vars=c("submitter_id","chromosome","mutation_position","race","Cancer"),na.rm = TRUE)
  
  #change variable names
  from_VCF_melted$variable <- gsub("AD_total_VCF","REF+ALT alleles",from_VCF_melted$variable)
  from_VCF_melted$variable <- gsub("AD_ref_VCF","REF alleles",from_VCF_melted$variable)
  from_VCF_melted$variable <- gsub("AD_alt_VCF","ALT alleles",from_VCF_melted$variable)
  
  #reorder
  from_VCF_melted$variable <- factor(from_VCF_melted$variable, levels = c("REF+ALT alleles","REF alleles","ALT alleles"),ordered=TRUE) 
  
  ggplot(from_VCF_melted, aes(x=factor(race),y=log(value+1,base=2),fill=race)) + xlab("") + ylab("log2(Number of reads per patient)") + facet_grid(Cancer~variable) +
    geom_boxplot() +  labs(title=paste0("Exome read depth of *SNP* mutations in ", GENE,"\nfor patients WITH these ",GENE," mutations in VCF\n(mutations identified from VCFs; read depth reported in MAF)")) + 
    theme(strip.text = element_text(size = 18,face='bold'),
          axis.text.x = element_blank(),
          axis.text.y = element_text(size=18),
          axis.title.y = element_text(size=20,face='bold',margin=margin(t=0,r=10,b=0,l=0)),
...
#may need to change aesthetics of plots below to match those above
