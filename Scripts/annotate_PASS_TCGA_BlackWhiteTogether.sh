...
INPUT=${INPUT_DIR}/${TISSUE}_BlackWhiteTogether_final_VQSR_chr.vcf
...
perl /research/bsi/projects/PI/tertiary/Asmann_Yan_wangy3/s212975.Wickland_Immunomics/tools/annovar/table_annovar.pl ${INPUT_PASS} humandb -buildver hg38 --out ${INPUT_PASS} -remove -protocol refGene,dbnsfp30a,EUR.sites.2015_08,AFR.sites.2015_08 -operation gx,f,f,f -nastring . -vcfinput " > /home/mayo/m187735/s212975.Wickland_Immunomics/commands_to_qsub/TCGA/annotate_PASS_TCGA/${PROJECT}/${CANCER_NAME}/BlackWhiteTogether.qsub




rm /research/bsi/projects/PI/tertiary/Asmann_Yan_wangy3/s212975.Wickland_Immunomics/commands_to_qsub/TCGA/annotate_PASS_TCGA/${PROJECT}/${CANCER_NAME}/BlackWhiteTogether*err 2> /dev/null
rm /research/bsi/projects/PI/tertiary/Asmann_Yan_wangy3/s212975.Wickland_Immunomics/commands_to_qsub/TCGA/annotate_PASS_TCGA/${PROJECT}/${CANCER_NAME}/BlackWhiteTogether*out 2> /dev/null

for file in /research/bsi/projects/PI/tertiary/Asmann_Yan_wangy3/s212975.Wickland_Immunomics/commands_to_qsub/TCGA/annotate_PASS_TCGA/${PROJECT}/${CANCER_NAME}/BlackWhiteTogether*qsub; do qsub -l h_vmem=20G -e /home/mayo/m187735/s212975.Wickland_Immunomics/commands_to_qsub/TCGA/annotate_PASS_TCGA/${PROJECT}/${CANCER_NAME}/`basename ${file} .qsub`.err -o /home/mayo/m187735/s212975.Wickland_Immunomics/commands_to_qsub/TCGA/annotate_PASS_TCGA/${PROJECT}/${CANCER_NAME}/`basename ${file} .qsub`.out -q 1-day -pe threaded 4 $file;done;
