...
PLOT_BY_RACE_AND_SAMPLING_SITE <- function(MAPPED_READS_OR_ALL_READS,CANCER_TYPES,EXOME_OR_RNASEQ){
  ...
  if (CANCER_TYPES=='all'){
    ...
    if(MAPPED_READS_OR_ALL_READS != 'mapped_BRCA_reads_within_nimblegen_capture'){
      ...
      for (CANCER_NAME in CANCER_LIST){
        ...
        master_table <- read.table(paste0('/home/mayo/m187735/s212975.Wickland_Immunomics/processing/TCGA/processed_data/overall_read_depths_and_sample_information/',CANCER_NAME,'_',MAPPED_READS_OR_ALL_READS,'_',EXOME_OR_RNASEQ,'_master_table.txt'),header=TRUE,sep='\t')
  ...
  if (CANCER_TYPES != 'all'){
    ...
    master_table_master <- read.table(paste0('/home/mayo/m187735/s212975.Wickland_Immunomics/processing/TCGA/processed_data/overall_read_depths_and_sample_information/',CANCER_NAME,'_',MAPPED_READS_OR_ALL_READS,'_',EXOME_OR_RNASEQ,'_master_table.txt'),header=TRUE,sep='\t')
  ...
  #set order for race 
  master_table_master <- subset(master_table_master, (race == 'White' | race == 'Black'))
  master_table_master$race <- factor(master_table_master$race, levels = c("White","Black"),ordered=TRUE)
  ...
  master_table_master$CAPTURE_KIT_simp <- factor(master_table_master$CAPTURE_KIT_simp, levels=c('Agilent SureSelect Human All Exon v2','Custom V2 Exome Bait','NimbleGen hg18 Exome v2','NimbleGen SeqCap EZ Exome v2','NimbleGen SeqCap EZ Exome v3','Roche SeqCap EZ HGSC VCRome'),ordered=TRUE)
...
##################################
#if sample called by two kits, just consider as having been captured by smallest
CAPTURE_TABLE <- (droplevels(MAPPED_READS_FOR_PLOTTING[,c('race','submitter_id','tissue.definition','Cancer','CAPTURE_KIT_FIXED')]) %>%
                    dplyr::group_by(Cancer,tissue.definition,race,CAPTURE_KIT_FIXED,.drop=FALSE) %>%
...
CAPTURE_TABLE_WHITE <- subset(CAPTURE_TABLE, race=='White')
names(CAPTURE_TABLE_WHITE)[4:5] <-c( "Primary Tumor White","Patient Germline White")

CAPTURE_TABLE_BLACK <- subset(CAPTURE_TABLE, race=='Black')
...
CAPTURE_TABLE$CAPTURE_KIT_simp <-  ifelse(grepl('hg18',CAPTURE_TABLE$CAPTURE_KIT_FIXED),'NimbleGen hg18 Exome v2',ifelse(grepl('v3.0',CAPTURE_TABLE$CAPTURE_KIT_FIXED),'NimbleGen SeqCap EZ Exome v3',ifelse(grepl('v2.0',CAPTURE_TABLE$CAPTURE_KIT_FIXED),'NimbleGen SeqCap EZ Exome v2',ifelse(grepl('V2.0',CAPTURE_TABLE$CAPTURE_KIT_FIXED),'NimbleGen SeqCap EZ Exome v2',ifelse(grepl('Custom',CAPTURE_TABLE$CAPTURE_KIT_FIXED),'Custom V2 Exome Bait',ifelse(grepl('Rome',CAPTURE_TABLE$CAPTURE_KIT_FIXED),'Roche SeqCap EZ HGSC VCRome',as.character(CAPTURE_TABLE$CAPTURE_KIT_FIXED))))))) 

CAPTURE_TABLE$CAPTURE_KIT_simp <- factor(CAPTURE_TABLE$CAPTURE_KIT_simp, levels=c('Agilent SureSelect Human All Exon v2','Custom V2 Exome Bait','NimbleGen hg18 Exome v2','NimbleGen SeqCap EZ Exome v2','NimbleGen SeqCap EZ Exome v3','Roche SeqCap EZ HGSC VCRome'),ordered=TRUE)
...
PLOT_OVERALL_READ_DEPTHS_BY_RACE <- function(MAPPED_READS_OR_ALL_READS,FIG_NUMBER){
  ...
  if (MAPPED_READS_OR_ALL_READS=='all_reads'){DF_TO_PLOT <- ALL_READS_FOR_PLOTTING; TITLE <- 'Total # of reads per exome'; UNITS <- 'millions'; N_POSITION=-50000000}
  if (MAPPED_READS_OR_ALL_READS=='mapped_reads'){DF_TO_PLOT <- MAPPED_READS_FOR_PLOTTING; TITLE <- '# of mapped reads per exome'; UNITS <- 'millions'; N_POSITION=-50000000} 
  if (MAPPED_READS_OR_ALL_READS=='unmapped_reads'){DF_TO_PLOT <- UNMAPPED_READS_FOR_PLOTTING; TITLE <- '# of unmapped reads per exome'; UNITS <- 'thousands'; N_POSITION=10} 
  ...
  Counts <- aggregate(read_depth ~ tissue.definition + Cancer + race, DF_TO_PLOT, length)
  Means <- aggregate(read_depth ~ tissue.definition + Cancer + race, DF_TO_PLOT, mean)
  if(MAPPED_READS_OR_ALL_READS=='all_reads' | MAPPED_READS_OR_ALL_READS=='mapped_reads'){Means$read_depth <- sprintf("%1.1f",round(Means$read_depth,1)/1000000); COORD_TOP=10600000;COORD_BOT=6300000}
  ...
  tumor_label_counts <- data.frame(race='White',tissue.definition=c('Primary Tumor'), read_depth=c(14000000),Cancer=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'),label=c('n','','','','','',''))
  tumor_label_counts$Cancer <- factor(tumor_label_counts$Cancer, levels=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'))
  germline_label_counts <- data.frame(race='White',tissue.definition=c('Patient Germline'), read_depth=c(14000000),Cancer=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'),label=c('n','','','','','',''))
  germline_label_counts$Cancer <- factor(germline_label_counts$Cancer, levels=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'))
  
  tumor_label_mean <- data.frame(race='White',tissue.definition=c('Primary Tumor'), read_depth=c(14000000),Cancer=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'),label=c('bar(x)','','','','','',''))
  tumor_label_mean$Cancer <- factor(tumor_label_mean$Cancer, levels=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'))
  germline_label_mean <- data.frame(race='White',tissue.definition=c('Patient Germline'), read_depth=c(14000000),Cancer=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'),label=c('bar(x)','','','','','',''))
  germline_label_mean$Cancer <- factor(germline_label_mean$Cancer, levels=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'))
  
  DF_TO_PLOT$Cancer <- factor(DF_TO_PLOT$Cancer, levels=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'))
  
  all_reads_by_race <- ggplot(DF_TO_PLOT, aes(x=factor(race),y=read_depth,fill=race)) + xlab("") + ylab(paste0("# of reads per exome (in ",UNITS, ")")) + facet_grid(tissue.definition~Cancer,labeller = label_wrap_gen(width=21)) + labs(title=TITLE) + 
    ...
    {if(MAPPED_READS_OR_ALL_READS=='all_reads' | MAPPED_READS_OR_ALL_READS=='mapped_reads') { coord_cartesian(ylim=c(17000000,400000000),clip='off')}}    +  
    ...
    #means comparisons
    stat_compare_means(size=7,method='t.test',mapping=aes(label=..p.signif..),
    ...
    {if(MAPPED_READS_OR_ALL_READS=='all_reads' | MAPPED_READS_OR_ALL_READS=='mapped_reads') {scale_y_continuous(expand = expansion(mult = c(0, 0.30)),labels=unit_format(unit = "",scale = 1e-6,accuracy=1,big.mark = ','),trans=pseudo_log_trans(base=2),breaks = c(25000000,100000000,400000000))}
      
      else if (MAPPED_READS_OR_ALL_READS=='unmapped_reads') {scale_y_continuous(expand = expansion(mult = c(.15, 0.30)),labels=unit_format(unit = "",scale = 1e-3,accuracy=1,big.mark = ','),trans=pseudo_log_trans(base=2),breaks = c(5000,100000,3000000))}
...
Counts_TP <- aggregate(Tumor_Purity ~ tissue.definition + Cancer + race, MAPPED_READS_FOR_PLOTTING, length)

MAPPED_READS_FOR_PLOTTING$Facet <- '' #add empty facet strip to line up plots

Tumor_Purity <- ggplot(MAPPED_READS_FOR_PLOTTING, aes(fill=race,x=race, y=Tumor_Purity)) +   facet_grid(Facet~Cancer) + theme(strip.text = element_text(face="bold", size=8)) + 
  geom_boxplot(fatten=2,outlier.shape=NA)+  ylab("Tumor purity") +  xlab("") + labs(title = ("Tumor purity per exome")) +  
  #geom_point() + 
...
#OUTPUT FIGURE 1
pdf(paste0("/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIG1.pdf"),height=13.5,width=19)
...
#OUTPUT FIGURE S1
pdf(paste0("/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIGS1.pdf"),height=17.5,width=19)
...
PLOT_OVERALL_READ_DEPTHS_BY_RACE_AND_CENTER <- function(MAPPED_READS_OR_ALL_READS){
  ...
  seq_site_counts <- seq_site[,c('submitter_id','race','Sequencing_Center_number','read_depth','Cancer','tissue.definition')] %>%
    #  tidyr::complete(sample_name, HLA_type,peptide_class, fill = list(number_of_unique_alleles = 0)) %>% #retain zero-counts
    dplyr::group_by(Cancer,tissue.definition,Sequencing_Center_number,race,.drop=FALSE) %>%
    dplyr::summarise(number_of_samples=dplyr::n()) 
  
  seq_site_counts <- seq_site_counts %>% spread(race, number_of_samples)
  ...
  seq_site$Race_tissue <- ifelse(seq_site$race=='Black' & seq_site$tissue.definition=='Primary Tumor','Black tumor',ifelse(seq_site$race=='White' & seq_site$tissue.definition=='Primary Tumor','White tumor',ifelse(seq_site$race=='Black' & seq_site$tissue.definition=='Patient Germline','Black germline',ifelse(seq_site$race=='White' & seq_site$tissue.definition=='Patient Germline','White germline',NA))))
  ...
  for (NUMBER in c(names(table(DF_TO_PLOT$Sequencing_Center_number)[1]),names(table(DF_TO_PLOT$Sequencing_Center_number)[2]),
    ...
    if(length(unique(SEQSITES_site$Cancer))==2){SCALE=20}
    ...
    SEQSITES_site_plot <- ggplot(SEQSITES_site, aes(x=factor(Race_tissue), y=read_depth,fill=Race_tissue)) + xlab("") + ylab("") + facet_grid(Sequencing_Center_number~Cancer,scales='free',drop=FALSE) + #in 'fill' argument can change factor order
      ...
      geom_boxplot(fatten=2,outlier.shape=NA) +labs(title = "") +
      theme(plot.margin=unit(c(-0.5,SCALE,-0.5,0), "cm"),
            ...
            legend.text=element_text(size=37), legend.title=element_blank(),
      ...
      stat_compare_means(size=11,method='t.test',mapping=aes(label=..p.signif..),
      ...
      scale_y_continuous(expand = expansion(mult = c(0.07, 0.205)),labels=unit_format(unit = "",scale = 1e-6,accuracy=1),trans=pseudo_log_trans(base=2),breaks = c(25000000,50000000,100000000,200000000)) +  scale_x_discrete(drop=FALSE) + #drop false to keep blank spots
      ...
      coord_cartesian(ylim=c(22000000,NA),clip='off') + 
      
      #add text annotation
      {if (NUMBER=='Center 1'){geom_text(data = Counts_SC,aes(label=read_depth, y=min(SEQSITES_site$read_depth)-4500000),size=9) }} + 
      {if (NUMBER!='Center 1'){geom_text(data = Counts_SC,aes(label=read_depth, y=min(SEQSITES_site$read_depth)-6000000),size=9) }}  
  ...
  figure <- annotate_figure(figure,left = text_grob("# of reads per exome (in millions)",size=40,rot = 90)) + theme(plot.margin = margin(b=0.75,l=0.5,t=0.75,r=2, "cm")) #incr marrgin at bottom
...
#OUTPUT FIGURE S2
pdf(paste0('/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIGS2.pdf'),height=20,width=22)
...
PLOT_OVERALL_READ_DEPTHS_BY_RACE_AND_SITE <- function(MAPPED_READS_OR_ALL_READS){
  ...
  #calcualte number of black, white samples per site
  source_site_counts <- source_site[,c('submitter_id','race','Source_Site','read_depth','Cancer','tissue.definition')] %>%
    #  tidyr::complete(sample_name, HLA_type,peptide_class, fill = list(number_of_unique_alleles = 0)) %>% #retain zero-counts
    dplyr::group_by(Cancer,tissue.definition,Source_Site,race,.drop=FALSE) %>%
    dplyr::summarise(number_of_samples=dplyr::n()) 
  ...
  #new column containing race and tissue
  source_site$Race_tissue <- ifelse(source_site$race=='Black' & source_site$tissue.definition=='Primary Tumor','Black tumor',ifelse(source_site$race=='White' & source_site$tissue.definition=='Primary Tumor','White tumor',ifelse(source_site$race=='Black' & source_site$tissue.definition=='Patient Germline','Black germline',ifelse(source_site$race=='White' & source_site$tissue.definition=='Patient Germline','White germline',NA))))
  ...
  for (CANCER_NAME in c(names(table(DF_TO_PLOT$Cancer)[1]),names(table(DF_TO_PLOT$Cancer)[2]),
    ...
    if(length(unique(source_site_cancer$Source_Site_number))==10){SCALE=1.50}
    ...
    if(length(unique(source_site_cancer$Source_Site_number))==5){SCALE=22.5}
    if(length(unique(source_site_cancer$Source_Site_number))==4){SCALE=71}
    if(length(unique(source_site_cancer$Source_Site_number))==3){SCALE=82.5}
    if(length(unique(source_site_cancer$Source_Site_number))==2){SCALE=93.75}
    if(length(unique(source_site_cancer$Source_Site_number))==1){SCALE=54.50}
    ...
    source_site_cancer_plot <- ggplot(source_site_cancer, aes(x=factor(Race_tissue), y=read_depth,fill=Race_tissue)) + xlab("") + ylab("") + facet_grid(Cancer~Source_Site_number3,scales='free',drop=FALSE) + #in 'fill' argument can change factor order
      ...
      geom_boxplot(outlier.shape=NA,fatten=2) +labs(title = "") +
      theme(plot.margin=unit(c(-0.5,SCALE,-0.5,0), "cm"),
            plot.title = element_text(size=46,hjust=0.5),
      ...
      stat_compare_means(size=13,method='t.test',mapping=aes(label=..p.signif..),
                         comparisons = list(c("Black tumor","White tumor")),bracket.size = NA,tip.length = 0,color='black',
                         symnum.args=list(cutpoints = c(0, 0.0001, 0.001, 0.01, 0.05, 1), symbols = c("p<0.0001", "p<0.001", "p<0.01", "p<0.05", ""))) +
      ...
      {if(CANCER_NAME=='PRAD' | CANCER_NAME=='LUAD'){scale_y_continuous(expand = expansion(mult = c(0.07, 0.25)),labels=unit_format(unit = "",scale = 1e-6,accuracy=1),trans=pseudo_log_trans(base=2),breaks = c(25000000,50000000,100000000,200000000))}} + 
      ...
      coord_cartesian(ylim=c(20000000,NA),clip='off') + 
      
      scale_fill_manual(values=c('#EF5350','#42A5F5','#FFCDD2','#BBDEFB'),labels=c('White tumor  ','Black tumor  ','White germline  ','Black germline   ')) + #MANUALLY SET FILL COLORS
      
      #add text annotation
      {if(CANCER_NAME=='PRAD' | CANCER_NAME=='LUAD'){geom_text(data = Counts_CS,aes(label=read_depth, y=min(source_site_cancer$read_depth)-8500000,vjust=0),size=14)}} + 
      
      {if (CANCER_NAME == 'UCEC' | CANCER_NAME == 'KIRC'){geom_text(data = Counts_CS,aes(label=read_depth, y=min(source_site_cancer$read_depth)-9750000,vjust=0),size=14)}} + 
      
      {if (CANCER_NAME == 'BRCA' | CANCER_NAME == 'COAD'){geom_text(data = Counts_CS,aes(label=read_depth, y=min(source_site_cancer$read_depth)-10500000,vjust=0),size=14)}}
  ...
  figure <- ggarrange(NULL,get(paste0("plot_",names(table(DF_TO_PLOT$Cancer)[1]))),NULL,get(paste0("plot_",names(table(DF_TO_PLOT$Cancer)[6]))),NULL,get(paste0("plot_",names(table(DF_TO_PLOT$Cancer)[2]))),NULL,get(paste0("plot_",names(table(DF_TO_PLOT$Cancer)[3]))),NULL,get(paste0("plot_",names(table(DF_TO_PLOT$Cancer)[4]))),NULL,get(paste0("plot_",names(table(DF_TO_PLOT$Cancer)[5]))),NULL,nrow=13,common.legend = TRUE, legend="bottom",heights=c(0.10,1.5,0.15,1.5,0.15,1.5,0.15,1.5,0.15,1.5,0.15,1.5,0.30))
  figure <- annotate_figure(figure,left = text_grob("# of reads per exome (in millions)",size=60,rot = 90)) + theme(plot.margin = margin(b=0.75,l=0.5,t=0.75,r=2, "cm")) #incr marrgin at bottom
...
#OUTPUT FIGURE S3
pdf(paste0('/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIGS3.pdf'),height=40,width=50)
...
PLOT_OVERALL_READ_DEPTHS_BY_RACE_AND_CAPKIT <- function(MAPPED_READS_OR_ALL_READS){
  ...
  #calcualte number of black, white samples per site
  CAPKITS_counts <- CAPKITS[,c('submitter_id','race','CAPTURE_KIT_simp','read_depth','Cancer','tissue.definition')] %>%
    #  tidyr::complete(sample_name, HLA_type,peptide_class, fill = list(number_of_unique_alleles = 0)) %>% #retain zero-counts
    dplyr::group_by(Cancer,tissue.definition,CAPTURE_KIT_simp,race,.drop=FALSE) %>%
    dplyr::summarise(number_of_samples=dplyr::n()) 
  ...
  CAPKITS <- CAPKITS[paste(CAPKITS$Cancer, CAPKITS$CAPTURE_KIT_simp, CAPKITS$tissue.definition, CAPKITS$race) %in% paste(CAPKITS_counts$Cancer, CAPKITS_counts$CAPTURE_KIT_simp, CAPKITS_counts$tissue.definition,CAPKITS_counts$race),]
  
  CAPKITS <- merge(CAPKITS,CAPKITS_numbers, by='CAPTURE_KIT_simp')
  
  
  #new column containing race and tissue
  CAPKITS$Race_tissue <- ifelse(CAPKITS$race=='Black' & CAPKITS$tissue.definition=='Primary Tumor','Black tumor',ifelse(CAPKITS$race=='White' & CAPKITS$tissue.definition=='Primary Tumor','White tumor',ifelse(CAPKITS$race=='Black' & CAPKITS$tissue.definition=='Patient Germline','Black germline',ifelse(CAPKITS$race=='White' & CAPKITS$tissue.definition=='Patient Germline','White germline',NA))))
  ...
  for (CANCER_NAME in c(names(table(DF_TO_PLOT$Cancer)[1]),names(table(DF_TO_PLOT$Cancer)[2]),
    ...
    CAPKITS_cancer_plot <- ggplot(CAPKITS_cancer, aes(x=factor(Race_tissue), y=read_depth,fill=Race_tissue)) + xlab("") + ylab("") + facet_grid(Cancer~CAPTURE_KIT_simp,scales='free',drop=FALSE,labeller = label_wrap_gen(width=20)) + #in 'fill' argument can change factor order
      ...
      theme(plot.margin=unit(c(-0.5,SCALE,-0.5,0), "cm"),
            ...
            axis.title.y = element_text(size=38,margin = margin(t = 0, r = 5, b = 0, l = 0)),
            axis.title.x = element_text(margin = margin(t = 15, r = 10, b = 5, l = 10)),
            axis.text.x = element_blank(),
            axis.ticks.x= element_blank(),
            axis.text.y = element_text(size=40,margin=margin(r=-1)),
            strip.background = element_blank(),
            panel.background = element_rect(color = 'black',fill=NA),
            panel.grid.major = element_blank(),
            panel.grid.minor = element_blank(),
            legend.spacing.x=unit(0.65,"cm"),
            legend.key.size = unit(4,"line"),
            legend.text=element_text(size=55), legend.title=element_blank(),
            legend.background = element_rect(linetype='solid', colour='black')) + guides(color = guide_legend(override.aes = list(size=30))) +
      ...
      scale_y_continuous(expand = expansion(mult = c(0.07, 0.25)),labels=unit_format(unit = "",scale = 1e-6,accuracy=1),trans=pseudo_log_trans(base=2),breaks = c(25000000,50000000,100000000,200000000)) +  scale_x_discrete(drop=FALSE) + #drop false to keep blank spots
      {if (CANCER_NAME != 'BRCA'){coord_cartesian(ylim=c(32500000,NA),clip='off')}} + 
      {if (CANCER_NAME == 'BRCA'){coord_cartesian(ylim=c(32500000,300000000),clip='off')}} + 
      
      #add text annotation
      {if(CANCER_NAME=='BRCA' | CANCER_NAME=='COAD'){geom_text(data = Counts_CK,aes(label=read_depth, y=min(CAPKITS_cancer$read_depth)-13500000,vjust=0),size=12)}} + 
      
      {if(CANCER_NAME=='PRAD'){geom_text(data = Counts_CK,aes(label=read_depth, y=min(CAPKITS_cancer$read_depth)-11000000,vjust=0),size=12)}} + 
      
      
      {if (CANCER_NAME == 'UCEC' | CANCER_NAME == 'KIRC' | CANCER_NAME == 'LUAD'){geom_text(data = Counts_CK,aes(label=read_depth, y=min(CAPKITS_cancer$read_depth)-13500000,vjust=0),size=12)}}
  ...
  figure <- ggarrange(NULL,get(paste0("plot_",names(table(DF_TO_PLOT$Cancer)[1]))),NULL,get(paste0("plot_",names(table(DF_TO_PLOT$Cancer)[6]))),NULL,get(paste0("plot_",names(table(DF_TO_PLOT$Cancer)[2]))),NULL,get(paste0("plot_",names(table(DF_TO_PLOT$Cancer)[3]))),NULL,get(paste0("plot_",names(table(DF_TO_PLOT$Cancer)[4]))),NULL,get(paste0("plot_",names(table(DF_TO_PLOT$Cancer)[5]))),NULL,nrow=13,common.legend = TRUE, legend="bottom",heights=c(0.08,1,0.12,1,0.12,1,0.12,1,0.12,1,0.12,1,0.20))
  figure <- annotate_figure(figure,left = text_grob("# of reads per exome (in millions)",size=60,rot = 90)) + theme(plot.margin = margin(b=0.75,l=0.5,t=0.75,r=2, "cm")) #incr marrgin at bottom
...
#OUTPUT FIGURE S4
pdf(paste0('/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIGS4.pdf'),height=42,width=32)
ggarrange(CAPTURE_KIT_MAPPED_READS)
dev.off()


######################################
#READ COUNT PER BRCA BAM WITHIN NIMBLEGEN CAPTURE REGIONS ONLY
######################################
...
NIMBLEGEN_BY_CAPTURE_BRCA_OVERALL_READ_DEPTHS <- function(){
  ...
  #calculate summary statistics
  Counts <- aggregate(read_depth_per_mb_capkit ~ tissue.definition +CAPTURE_KIT_simp + race, DF_TO_PLOT, length)
  Means <- aggregate(read_depth_per_mb_capkit ~ tissue.definition + CAPTURE_KIT_simp + race, DF_TO_PLOT, mean)
  Means$read_depth_per_mb_capkit <- sprintf("%1.2f",round(Means$read_depth_per_mb_capkit,1)/1000000)
  
  #create labels for the annotated text below each facet row
  tumor_label_counts <- data.frame(race='White',tissue.definition=c('Primary Tumor'), read_depth_per_mb_capkit=c(140000),CAPTURE_KIT_simp=c('All NimbleGen kits','NimbleGen SeqCap EZ Exome v2','NimbleGen SeqCap EZ Exome v3','NimbleGen hg18 Exome v2'),label=c('n','','',''))
  #tumor_label_counts$Cancer <- factor(tumor_label_counts$Cancer, levels=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'))
  germline_label_counts <- data.frame(race='White',tissue.definition=c('Patient Germline'), read_depth_per_mb_capkit=c(140000),CAPTURE_KIT_simp=c('All NimbleGen kits','NimbleGen SeqCap EZ Exome v2','NimbleGen SeqCap EZ Exome v3','NimbleGen hg18 Exome v2'),label=c('n','','',''))
  #germline_label_counts$Cancer <- factor(germline_label_counts$Cancer, levels=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'))
  
  tumor_label_mean <- data.frame(race='White',tissue.definition=c('Primary Tumor'), read_depth_per_mb_capkit=c(140000),CAPTURE_KIT_simp=c('All NimbleGen kits','NimbleGen SeqCap EZ Exome v2','NimbleGen SeqCap EZ Exome v3','NimbleGen hg18 Exome v2'),label=c('bar(x)','','',''))
  #tumor_label_mean$Cancer <- factor(tumor_label_mean$Cancer, levels=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'))
  germline_label_mean <- data.frame(race='White',tissue.definition=c('Patient Germline'), read_depth_per_mb_capkit=c(140000),CAPTURE_KIT_simp=c('All NimbleGen kits','NimbleGen SeqCap EZ Exome v2','NimbleGen SeqCap EZ Exome v3','NimbleGen hg18 Exome v2'),label=c('bar(x)','','',''))
  #germline_label_mean$Cancer <- factor(germline_label_mean$Cancer, levels=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'))
  ...
  plot <- ggplot(DF_TO_PLOT, aes(x=factor(race),y=read_depth_per_mb_capkit,fill=race)) + xlab("") + ylab(paste0("# of reads per exome per Mb of capture region (in millions) ")) + facet_grid(tissue.definition~CAPTURE_KIT_simp,labeller = label_wrap_gen(width=19.5)) + 
    geom_boxplot(fatten=2) + 
...
#OUTPUT FIGURE 2
pdf(paste0("/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIG2.pdf"),height=9.5,width=13)
...
race_data <- ALL_READS_FOR_PLOTTING[c('submitter_id','race')]

#Create dataframe to hold MAF data for all cancers
MAF_all <- data.frame()

CANCER_LIST <- c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD')
...
race_MAF_depths <- subset(race_MAF_depths, (race=='Black' | race=='White'))
...
race_MAF_depths_melted <- reshape2::melt(race_MAF_depths, id.vars=c('submitter_id','Cancer','race','Tumor_Sample_Barcode','Matched_Norm_Sample_Barcode','Chromosome','Start_Position','End_Position','Hugo_Symbol','Reference_Allele','Tumor_Seq_Allele1','Tumor_Seq_Allele2','t_ref_count','t_alt_count','dbSNP_RS','Variant_Classification','Variant_Type','Center'))
...
race_MAF_depths_melted$Cancer <- factor(race_MAF_depths_melted$Cancer,levels=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD'),ordered=TRUE)
...
MAF_reads_by_race <- ggplot(race_MAF_depths_melted, aes(x=factor(race),y=value,fill=race)) + xlab("") + ylab("# of reads per position") + #facet_grid(Facet~Cancer,scales='free', labeller = label_wrap_gen(width=19)) + labs(title='Coverage per somatic variant position') + 
  ...
  #means comparisons
  stat_compare_means(size=6,method='t.test',mapping=aes(label=..p.signif..),
                     comparisons = list(c("Black","White")),bracket.size = NA,tip.length = 0,color='black',
                     #label.y=400000000, #adjust position of p-value
                     symnum.args=list(cutpoints = c(0, 0.0001, 0.001, 0.01, 0.05, 1), symbols = c("p<0.0001", "p<0.001", "p<0.01", "p<0.05", ""))) 
...
#note: EDIT MOSAICPLOT FUNCTION FIRST, BELOW!
trace(graphics:::mosaicplot, edit = TRUE, where = graphics)
...
for (TISSUE in c('Primary_Tumor','Patient_Germline')){
  for (FILE in list.files(paste0('/research/bsi/projects/PI/tertiary/Asmann_Yan_wangy3/s212975.Wickland_Immunomics/processing/TCGA/processed_data/DP_per_position_from_bams/BRCA/',TISSUE,'/',OUTPUT_NAME),pattern="*summary*",full.names = TRUE)){
...
ALL_SAMPLES_ALL <- ALL_SAMPLES


PLOT_MOSAIC <- function(WHICH_TISSUE){
  for (KIT in c('All_kits','hg18_nimblegen_exome_version_2','Nimblegen_SeqCap_EZ_Exome_v2','Nimblegen_SeqCap_EZ_Exome_v3')){
    
    if (KIT != 'All_kits'){ALL_SAMPLES <- subset(ALL_SAMPLES_ALL,CAPTURE_KIT==KIT)}
    if (KIT == 'All_kits'){ALL_SAMPLES <- ALL_SAMPLES_ALL}
    ...
    KIT <- gsub('hg18_nimblegen_exome_version_2','NimbleGen_hg18_Exome_v2',KIT)
    KIT <- gsub('Nimblegen','NimbleGen',KIT)
...
COMBINE_DATA <- function(PREFIX,CANCER_NAME,RACE){  
  ...
  for (TISSUE in c("Blood_Normal","Primary_Tumor")){
...
Black_MAFs_DPs_at_ALT_positions <- COMBINE_DATA('nimblegen_capture_Apr2','BRCA','BlackWhite_Black') #CHANGE FOR JOINT BLACK/WHITE TOGETHER!
...
White_MAFs_DPs_at_ALT_positions <- COMBINE_DATA('nimblegen_capture_Apr2','BRCA','BlackWhite_White') #CHANGE FOR JOINT BLACK/WHITE TOGETHER!
...
plot_MAF_by_coverage_level <- function(DF,INCLUDE_EQNS){
  ...
  plot <- ggplot(DF, aes(x=as.numeric(as.character(BAF_1kGenomes)), y=as.numeric(as.character(BAF)))) + #facet_grid(rows=vars(Tissue),cols = vars(Race)) + 
    ...
    facet_wrap(~ Race) +
    geom_point(shape=1,size=0.5,alpha=0.1,aes(color=(fct_reorder(DF$DP_group, DF$variable2, min)))) + ylab('TCGA BRCA cohort MAF') + xlab("1000 Genomes population MAF") + labs(title="Minor allele frequency in common capture region:\nTCGA BRCA germline vs. 1000 Genomes") + guides(color=guide_legend(title="Coverage",size=12)) + 
    
    geom_smooth(linetype='solid',position=position_jitter(0.000),method='lm', formula= y~x,aes(color=(fct_reorder(DF$DP_group, DF$variable2, min))),se=FALSE,size=1.0)   +
    ...
    theme(strip.text.x = element_text(size = 30),#,hjust=0),
          ...
          plot.tag.position = c(0.01, 0.98),
          legend.position=c(0.11,0.835),legend.direction='vertical',
    ...
    scale_shape_discrete(name = "Coverage", 
                         labels = expression("" >= 40, "11-39", "" <= 10,"" <= 5)) + 
...
THOU_GENOMES_CAPTURE <- ggplot(Thousand_Genomes_Common_capture_MELTED, aes(x=as.factor(variable),y=value*100,fill=variable)) +  
  geom_boxplot(fatten=4) + ylab('MAF') + xlab('') + labs(title='Minor allele frequency in common capture region:\n1000 Genomes European vs. 1000 Genomes African') + #geom_point() + 
  ...
  #means comparisons
  stat_compare_means(size=10,method='t.test',mapping=aes(label=..p.signif..),
...
GENERATE_BAC_PLOT <- function(MIN_READS,DATASET,X_POS,Y_POS){
  ...
  BAC_plot <- ggplot(MAF_subset, aes(x=BAC))+ xlab('BAC') + ylab('Density') +  facet_grid(MIN_READS~TYPE) +
    ...
    #geom_text(aes(x=1/MINIMUM_READS,y=AUC_coords[which.min(abs(1/MINIMUM_READS - AUC_coords$x)),]$y,label = paste0(round(as.numeric(ecdf(MAF_subset$BAC)(1/MINIMUM_READS)-ecdf(MAF_subset$BAC)(0)),digits=3)*100,"% of variants potentially missed\n (min. BAC = ",1/MINIMUM_READS,")"), vjust = -0.20,hjust=-0.050),size=5) +
    ...
    scale_y_continuous(expand = expansion(mult = c(.05, 0.15))) + ylab("") + 
...
MAF_protein_altering <- MAF_protein_altering[,c('submitter_id','race','Chromosome','Start_Position','Hugo_Symbol','t_depth','t_ref_count','t_alt_count','Variant_Classification','Cancer')]
...
BAC5_all <- GENERATE_BAC_PLOT(5,MAF_protein_altering,0.55,1.4)
BAC10_all <-  GENERATE_BAC_PLOT(10,MAF_protein_altering,0.55,1.4)
BAC40_all <-  GENERATE_BAC_PLOT(40,MAF_protein_altering,0.55,1.4)

BAC5_BRCA <- GENERATE_BAC_PLOT(5,BRCA_protein_altering,0.20,2.5)
BAC10_BRCA <-  GENERATE_BAC_PLOT(10,BRCA_protein_altering,0.20,2.5)
BAC40_BRCA <-  GENERATE_BAC_PLOT(40,BRCA_protein_altering,0.20,2.5)
...
#arrange cancer order
somatic_with_vcf_and_normal_info_melted$Cancer <- factor(somatic_with_vcf_and_normal_info_melted$Cancer,levels=c('BRCA','PRAD','LUAD','UCEC','KIRC','COAD','KIRP'),ordered=TRUE)

#plot
Counts_QUAL <- aggregate(value ~ variable + Cancer + race, somatic_with_vcf_and_normal_info_melted, length)

MAF_quality_by_race <- ggplot(somatic_with_vcf_and_normal_info_melted, aes(x=factor(race),y=as.numeric(value),fill=race)) + xlab("") + ylab("Log-likelihood ratio per position") + facet_grid(variable~Cancer,scales='free', labeller = label_wrap_gen(width=19)) + labs(title='Confidence of somatic mutation call per variant position') + 
  geom_boxplot(fatten=2) + 
  theme(strip.text.x = element_text(size = 24),#,hjust=0),
        strip.text.y = element_text(size = 22),
        axis.text.x = element_blank(),
        ...
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.spacing.y=unit(3,"lines"), #to control distance betweeen facet rows
        legend.position = 'bottom',legend.direction='horizontal',
        legend.text=element_text(size=32),
        legend.background = element_rect(linetype='solid', colour='black'),
  ...
  #means comparisons
  stat_compare_means(size=8,method='t.test',mapping=aes(label=..p.signif..),
                     comparisons = list(c("Black","White")),bracket.size = NA,tip.length = 0,color='black',
                     #label.y=400000000, #adjust position of p-value
                     symnum.args=list(cutpoints = c(0, 0.0001, 0.001, 0.01, 0.05, 1), symbols = c("p<0.0001", "p<0.001", "p<0.01", "p<0.05", ""))) +
...
#OUTPUT FIGURE S5
pdf(paste0("/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIGS5.pdf"),height=9,width=19)
...
png(paste0('/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIGS6.png'),height=17,width=13,res = 1400,units = 'in')
