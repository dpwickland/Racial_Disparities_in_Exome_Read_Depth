...
PLOT_BY_RACE_AND_SAMPLING_SITE <- function(MAPPED_READS_OR_ALL_READS,CANCER_TYPES,EXOME_OR_RNASEQ){
  ...
  master_table_master$race_PCA <- factor(master_table_master$race_PCA, levels = c("African","European"),ordered=TRUE)
...
PLOT_OVERALL_READ_DEPTHS_BY_RACE <- function(MAPPED_READS_OR_ALL_READS,FIG_NUMBER){
  ...
  if (MAPPED_READS_OR_ALL_READS=='all_reads'){DF_TO_PLOT <- ALL_READS_FOR_PLOTTING; TITLE <- 'Total number of reads per exome'; UNITS <- 'Millions'; N_POSITION=-log(50000000,base=2)}
  if (MAPPED_READS_OR_ALL_READS=='mapped_reads'){DF_TO_PLOT <- MAPPED_READS_FOR_PLOTTING; TITLE <- 'Number of mapped reads per exome'; UNITS <- 'Millions'; N_POSITION=-log(50000000,base=2)} 
  if (MAPPED_READS_OR_ALL_READS=='unmapped_reads'){DF_TO_PLOT <- UNMAPPED_READS_FOR_PLOTTING; TITLE <- 'Number of unmapped reads per exome'; UNITS <- 'Thousands'; N_POSITION=log(10,base=2)} 
  ...
  if(MAPPED_READS_OR_ALL_READS=='all_reads' | MAPPED_READS_OR_ALL_READS=='mapped_reads'){Means$read_depth <- sprintf("%1.1f",round(Means$read_depth,1)/1000000); COORD_TOP=log(7200000,base=2);COORD_BOT=log(3990000,base=2)}
  if(MAPPED_READS_OR_ALL_READS=='unmapped_reads'){Means$read_depth <- sprintf("%1.1f",round(Means$read_depth,1)/1000);COORD_TOP=log(300,base=2);COORD_BOT=log(61,base=2)}
  ...
  #############CALCULATE P-VALUES
  #calculate
  STATS_FOR_PLOT <- DF_TO_PLOT %>% group_by(Cancer,tissue.definition) %>%
    wilcox_test(read_depth ~ race_PCA) %>% add_xy_position(x = 'race_PCA',scales=c('free'))
  
  #Modify the Y position manually so that it's same for all facets
  STATS_FOR_PLOT$y.position <- log(max(DF_TO_PLOT$read_depth)*1.25,base=2)
  
  #adhere to JNCI conventions for p-values and decimals; also, add asterisk for significant
  STATS_FOR_PLOT$p <- ifelse(STATS_FOR_PLOT$p < 0.001, "p<0.001*",
                             ifelse(STATS_FOR_PLOT$p < 0.05 & STATS_FOR_PLOT$p >= 0.045,round(STATS_FOR_PLOT$p, digits=3),
                                    ifelse(STATS_FOR_PLOT$p <= 0.99 & STATS_FOR_PLOT$p >= 0.01,sprintf("%.2f",round(STATS_FOR_PLOT$p, digits=2)),
                                           ifelse(STATS_FOR_PLOT$p <= 0.009 & STATS_FOR_PLOT$p >= 0.001,round(STATS_FOR_PLOT$p, digits=3),round(STATS_FOR_PLOT$p,digits=1)))))
  
  
  STATS_FOR_PLOT$p <- ifelse(STATS_FOR_PLOT$p < 0.05, paste0(STATS_FOR_PLOT$p, "*"),STATS_FOR_PLOT$p)
  STATS_FOR_PLOT$p <- ifelse(STATS_FOR_PLOT$p != "p<0.001*",paste0('p=',STATS_FOR_PLOT$p),STATS_FOR_PLOT$p)
  
  
  #############
  
  all_reads_by_race_PCA <- ggplot(DF_TO_PLOT, aes(x=factor(race_PCA),y=log(read_depth,base=2),fill=race_PCA)) + xlab("") + ylab(paste0(UNITS, " of reads per exome (log2 scale)")) + facet_grid(tissue.definition~Cancer,labeller = label_wrap_gen(width=21)) + labs(title=TITLE) + 
    ...
    theme(strip.text.x = element_text(size = 24),#,hjust=0),
          ...
          axis.text.y = element_text(size=22,margin=margin(r=3.5,l=3.5)),
          ...
          legend.key.size = unit(2.25,"line"),
    ...
    geom_text(data= tumor_label_counts,aes(label=label,y=COORD_TOP,vjust=0.2,hjust=10.1,fontface="bold"),size=7) +
    geom_text(data= tumor_label_mean,aes(label=label,y=COORD_BOT,vjust=0.2,hjust=12.30,fontface="bold"),size=7,parse=TRUE) + #}} +
    geom_text(data= tumor_label_mean,aes(label=label,y=COORD_BOT,vjust=0.2,hjust=12.30,fontface="bold"),size=7.05,parse=TRUE) + #}} +
    ...
    geom_text(data= germline_label_counts,aes(label=label,y=COORD_TOP,vjust=0.2,hjust=10.1,fontface="bold"),size=7) +
    geom_text(data= germline_label_mean,aes(label=label,y=COORD_BOT,vjust=0.2,hjust=12.30,fontface="bold"),size=7,parse=TRUE) + #}} +
    geom_text(data= germline_label_mean,aes(label=label,y=COORD_BOT,vjust=0.2,hjust=12.30,fontface="bold"),size=7.05,parse=TRUE) + #}} +
    
    
    #allow table outside plot margins
    {if(MAPPED_READS_OR_ALL_READS=='all_reads' | MAPPED_READS_OR_ALL_READS=='mapped_reads') { coord_cartesian(ylim=c(log(12000000,base=2),log(400000000,base=2)),clip='off')}}    +  
    {if(MAPPED_READS_OR_ALL_READS=='unmapped_reads') {coord_cartesian(ylim=c(log(200,base=2),log(35000000,base=2)),clip='off') }}    +  
    scale_fill_manual(values=c('#EF5350','#42A5F5'),labels=c('African ','European ')) + #MANUALLY SET FILL COLORS
    
    #means comparisons
    stat_pvalue_manual(STATS_FOR_PLOT,inherit.aes=FALSE,bracket.size = NA,tip.length = 0,label='p',size=7,hide.ns=FALSE,color='black') +
    
    {if(MAPPED_READS_OR_ALL_READS=='all_reads' | MAPPED_READS_OR_ALL_READS=='mapped_reads') {scale_y_continuous(expand = expansion(mult = c(0, 0.28)),breaks = c(log(25000000,base=2),log(50000000,base=2),log(100000000,base=2),log(200000000,base=2),log(400000000,base=2)),labels=c(25,50,100,200,400))}
      
      else if (MAPPED_READS_OR_ALL_READS=='unmapped_reads') {scale_y_continuous(expand = expansion(mult = c(0.05, 0.15)),breaks = c(log(5000,base=2),log(25000,base=2),log(100000,base=2),log(500000,base=2),log(2000000,base=2),log(10000000,base=2)),labels=c(5,25,100,500,"2,000","10,000"))}} +
    
    
    #allow table outside plot margins
    {if(MAPPED_READS_OR_ALL_READS=='all_reads' | MAPPED_READS_OR_ALL_READS=='mapped_reads') { coord_cartesian(ylim=c(log(12000000,base=2),log(400000000,base=2)),clip='off')}}    +  
    {if(MAPPED_READS_OR_ALL_READS=='unmapped_reads') {coord_cartesian(ylim=c(log(2500,base=2),log(35000000,base=2)),clip='off') }}   
...
OVERALL_READ_DEPTHS_BY_RACE_MAPPED_READS <- PLOT_OVERALL_READ_DEPTHS_BY_RACE('mapped_reads','Figure 1')
...
DF_TO_PLOT <- MAPPED_READS_FOR_PLOTTING
DF_TO_PLOT <- subset(DF_TO_PLOT, Tumor_Purity != 'Not reported')

Counts_TP <- aggregate(Tumor_Purity ~ tissue.definition + Cancer + race_PCA, DF_TO_PLOT, length)
...
DF_TO_PLOT$Tumor_Purity <- as.numeric(as.character(DF_TO_PLOT$Tumor_Purity))

#calculate
STATS_FOR_PLOT <- DF_TO_PLOT %>% group_by(Cancer) %>%
  wilcox_test(Tumor_Purity ~ race_PCA) %>% add_xy_position(x = 'race_PCA',scales=c('free'))

#Modify the Y position manually so that it's same for all facets
STATS_FOR_PLOT$y.position <- max(DF_TO_PLOT$Tumor_Purity)*1.25

#adhere to JNCI conventions for p-values and decimals; also, add asterisk for significant
STATS_FOR_PLOT$p <- ifelse(STATS_FOR_PLOT$p < 0.001, "<0.001*",
                           ifelse(STATS_FOR_PLOT$p < 0.05 & STATS_FOR_PLOT$p >= 0.045,round(STATS_FOR_PLOT$p, digits=3),
                                  ifelse(STATS_FOR_PLOT$p <= 0.99 & STATS_FOR_PLOT$p >= 0.01,sprintf("%.2f",round(STATS_FOR_PLOT$p, digits=2)),
                                         ifelse(STATS_FOR_PLOT$p <= 0.009 & STATS_FOR_PLOT$p >= 0.001,round(STATS_FOR_PLOT$p, digits=3),round(STATS_FOR_PLOT$p,digits=1)))))
...
Tumor_Purity <- ggplot(MAPPED_READS_FOR_PLOTTING, aes(fill=race_PCA,x=race_PCA, y=as.numeric(as.character(Tumor_Purity)))) +   facet_grid(Facet~Cancer) + theme(strip.text = element_text(face="bold", size=8)) + 
  ...
  theme(strip.text.x = element_text(size = 24,face='plain'),#,hjust=0),
        axis.text.x = element_blank(),
        axis.text.y = element_text(size=22,margin=margin(t=0,r=3.5,b=0,l=3.5)),
        axis.title.y = element_text(size=24,margin=margin(t=0,r=10,b=0,l=0)),
        axis.ticks.length=unit(.25, "cm"),
        axis.ticks.x = element_blank(),
        plot.title = element_text(hjust=0.5,size=30,face='bold',margin=margin(t=0,r=0,b=15,l=0)),
...
#OUTPUT FIGURE 1
pdf(paste0("/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIG1.pdf"),height=15,width=19,onefile = FALSE)
ggarrange(NULL,NULL,NULL,OVERALL_READ_DEPTHS_BY_RACE_MAPPED_READS,NULL,NULL,NULL,Tumor_Purity,NULL,NULL,nrow=5,ncol=2,common.legend = TRUE,hjust=00,legend = 'bottom',align='hv',widths=c(0.02,1),heights=c(0.1,0.9,0.07,0.50,0.11),labels=c('Figure 1','','A','','','','B','','',''),font.label = list(size = 32, color = "black"))  + theme(plot.margin = margin(b=0.5,l=0.1,t=0.1,r=0.1, "cm")) #incr marrgin at bottom
dev.off()

png(paste0("/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIGS2.png"),height=17.75,width=19,res=500,units='in')
ggarrange(NULL,NULL,NULL,OVERALL_READ_DEPTHS_BY_RACE_ALL_READS,NULL,NULL,NULL,OVERALL_READ_DEPTHS_BY_RACE_UNMAPPED_READS,NULL,NULL,nrow=5,ncol=2,common.legend = TRUE,hjust=00,legend = 'bottom',align='hv',widths=c(0.02,1),heights=c(0.1,0.9,0.12,0.90,0.11),labels=c('Supplementary Figure 2','','A','','','','B','','',''),font.label = list(size = 32, color = "black"))  + theme(plot.margin = margin(b=0.5,l=0.1,t=0.1,r=0.1, "cm")) #incr marrgin at bottom
dev.off()
...
PLOT_OVERALL_READ_DEPTHS_BY_RACE_AND_CENTER <- function(MAPPED_READS_OR_ALL_READS){
  ...
  seq_site$Race_tissue <- factor(seq_site$Race_tissue, levels=c('African tumor','European tumor','African germline','European germline'),ordered=TRUE)
  ...
  for (NUMBER in c(names(table(DF_TO_PLOT$Sequencing_Center_number)[1]),names(table(DF_TO_PLOT$Sequencing_Center_number)[2]),
    ...
    #############CALCULATE P-VALUES
    DF_TO_PLOT2 <- SEQSITES_site
    #calculate
    STATS_FOR_PLOT <- DF_TO_PLOT2 %>% group_by(Cancer) %>%
      wilcox_test(read_depth ~ Race_tissue) %>% add_xy_position(x = 'Race_tissue',scales=c('free'))
    
    #Modify the Y position manually so that it's same for all facets
    STATS_FOR_PLOT$y.position <- log(max(DF_TO_PLOT2$read_depth)*1.25,base=2)
    ...
    #only want certain comparisons
    STATS_FOR_PLOT <-  STATS_FOR_PLOT[(grepl("tumor",STATS_FOR_PLOT$group1) & grepl("tumor",STATS_FOR_PLOT$group2) | grepl("germline",STATS_FOR_PLOT$group1) & grepl("germline",STATS_FOR_PLOT$group2)),]
    
    
    SEQSITES_site_plot <-   ggplot(SEQSITES_site, aes(x=factor(Race_tissue), y=log(read_depth,base=2),fill=Race_tissue)) + xlab("") + ylab("") + facet_grid(Sequencing_Center_number~Cancer,scales='free',drop=FALSE) + #in 'fill' argument can change factor order
      ...
      theme(plot.margin=unit(c(-0.5,SCALE,0.25,0), "cm"),
            ...
            axis.ticks.length=unit(.25, "cm"),
            axis.ticks.x = element_blank(),
            axis.text.y = element_text(size=42,margin=margin(r=3.5,l=3.5)),
            strip.background = element_blank(),
            panel.background = element_rect(color = 'black',fill=NA),
            panel.grid.major = element_blank(),
            panel.grid.minor = element_blank(),
      ...
      #means comparisons
      stat_pvalue_manual(STATS_FOR_PLOT,inherit.aes=FALSE,bracket.size = NA,tip.length = 0,label='p',size=11,hide.ns=FALSE,color='black',y.position = log(450000000,base=2)) +
      
      
      scale_y_continuous(expand = expansion(mult = c(0.05, 0.20)),labels=c(25,50,100,200,400),breaks = c(log(25000000,base=2),log(50000000,base=2),log(100000000,base=2),log(200000000,base=2),log(400000000,base=2))) +  scale_x_discrete(drop=FALSE) + #drop false to keep blank spots
      scale_fill_manual(values=c('#EF5350','#42A5F5','#FFCDD2','#BBDEFB'),labels=c('African tumor  ','European tumor  ','African germline   ','European germline   ')) + #MANUALLY SET FILL COLORS 
      
      
      
      coord_cartesian(ylim=c(log(15000000,base=2),NA),clip='off') + 
      
      #add text annotation
      {if (NUMBER=='Center 1'){geom_text(data = Counts_SC,aes(label=read_depth, y=23.25),size=9) }} + 
      {if (NUMBER!='Center 1'){geom_text(data = Counts_SC,aes(label=read_depth, y=23.25),size=9) }}    
...
#OUTPUT FIGURE S3
png(paste0('/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIGS3.png'),height=21,width=16,res=500,units='in')
ggarrange(NULL,NULL,NULL,SEQ_CENTER_MAPPED_READS,nrow=2,ncol=2,common.legend=TRUE,legend='bottom',hjust=00,align='hv',widths=c(0.02,1),heights=c(0.04,1),labels=c('Supplementary Figure 3','','',''),font.label = list(size = 36,color='black')) + theme(plot.margin = margin(b=0.5,l=0.1,t=0.1,r=0.1, "cm")) 
dev.off()
...
PLOT_OVERALL_READ_DEPTHS_BY_RACE_AND_SITE <- function(MAPPED_READS_OR_ALL_READS){
  ...
  source_site$Race_tissue <- factor(source_site$Race_tissue, levels=c('African tumor','European tumor','African germline','European germline'))
  ...
  for (CANCER_NAME in c('BRCA','BRCA ','UCEC','PRAD','LUAD','COAD','KIRC')){  
    ...
    #############CALCULATE P-VALUES
    DF_TO_PLOT2 <- source_site_cancer
    #calculate
    STATS_FOR_PLOT <- DF_TO_PLOT2 %>% group_by(Source_Site_number3) %>%
      wilcox_test(read_depth ~ Race_tissue) %>% add_xy_position(x = 'Race_tissue',scales=c('free'))
    ...
    source_site_cancer_plot <- ggplot(source_site_cancer, aes(x=factor(Race_tissue), y=log(read_depth,base=2),fill=Race_tissue)) + xlab("") + ylab("") + facet_grid(Cancer~Source_Site_number3,scales='free',drop=FALSE) + #in 'fill' argument can change factor order
      ...
      #means comparisons
      stat_pvalue_manual(STATS_FOR_PLOT,inherit.aes=FALSE,bracket.size = NA,tip.length = 0,label='p',size=13,hide.ns=FALSE,color='black') +
      ...
      scale_y_continuous(expand = expansion(mult = c(0.07, 0.19)),breaks = round(seq(from=log(RANGE1,base=2),to=log(RANGE2,base=2),length.out=5),digits = 1),labels=round(2^round(seq(from=log(RANGE1,base=2),to=log(RANGE2,base=2),length.out=5),digits = 1),digits=-6)/1000000) + 
      
      #scale_y_continuous(expand = expansion(mult = c(0.07, 0.19)),breaks = round(seq(from=log(RANGE1,base=2),to=log(RANGE2,base=2),length.out=5),digits = 0),labels = round_any(round(seq(from=2^log(RANGE1,base=2),to=2^log(RANGE2,base=2),length.out=5),digits = -5),2^log(10000000,base=2))/1000000) + 
      
      
      
      
      {if(CANCER_NAME=='PRAD' ){coord_cartesian(ylim=c(log(28000000,base=2),NA),clip='off') }} + 
      {if(CANCER_NAME=='LUAD' ){coord_cartesian(ylim=c(log(40000000,base=2),NA),clip='off') }} + 
      {if(CANCER_NAME=='BRCA' | CANCER_NAME=='BRCA ' | CANCER_NAME=='UCEC' ){coord_cartesian(ylim=c(log(30000000,base=2),NA),clip='off') }} + 
      
      {if(CANCER_NAME=='KIRC' ){coord_cartesian(ylim=c(log(40000000,base=2),NA),clip='off') }} + 
      {if(CANCER_NAME=='COAD' ){coord_cartesian(ylim=c(log(16000000,base=2),NA),clip='off') }} + 
      
      #add text annotation
      {if(CANCER_NAME=='PRAD'){geom_text(data = Counts_CS,aes(label=read_depth, y=23.88,vjust=0),size=14)}} + 
      {if(CANCER_NAME=='LUAD'){geom_text(data = Counts_CS,aes(label=read_depth, y=24.72,vjust=0),size=14)}} + 
      {if(CANCER_NAME=='BRCA'){geom_text(data = Counts_CS,aes(label=read_depth, y=23.85,vjust=0),size=14)}} + 
      {if(CANCER_NAME=='BRCA '){geom_text(data = Counts_CS,aes(label=read_depth, y=23.85,vjust=0),size=14)}} + 
      {if(CANCER_NAME=='KIRC'){geom_text(data = Counts_CS,aes(label=read_depth, y=24.55,vjust=0),size=14)}} + 
      {if(CANCER_NAME=='COAD'){geom_text(data = Counts_CS,aes(label=read_depth, y=22.75,vjust=0),size=14)}} + 
      {if(CANCER_NAME=='UCEC'){geom_text(data = Counts_CS,aes(label=read_depth, y=24,vjust=0),size=14)}} 
...
#OUTPUT FIGURE S4
png(paste0('/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIGS4.png'),height=50,width=35,units='in',res=500)
ggarrange(NULL,NULL,NULL,COLLECTION_SITE_MAPPED_READS,nrow=2,ncol=2,common.legend=TRUE,legend='bottom',hjust=00,align='hv',widths=c(0.02,1),heights=c(0.03,1),labels=c('Supplementary Figure 4','','',''),font.label = list(size = 70,color='black')) + theme(plot.margin = margin(b=0.5,l=0.1,t=0.1,r=0.1, "cm")) 
dev.off()
...
PLOT_OVERALL_READ_DEPTHS_BY_RACE_AND_CAPKIT <- function(MAPPED_READS_OR_ALL_READS){
  ...
  CAPKITS$Race_tissue <- factor(CAPKITS$Race_tissue, levels=c('African tumor','European tumor','African germline','European germline'))
  ...
  for (CANCER_NAME in c(names(table(DF_TO_PLOT$Cancer)[1]),names(table(DF_TO_PLOT$Cancer)[2]),
    ...
    #############CALCULATE P-VALUES
    DF_TO_PLOT2 <- CAPKITS_cancer
    
    DF_TO_PLOT2 <- subset(DF_TO_PLOT2, CAPTURE_KIT_simp != 'Not reported')
    #calculate
    STATS_FOR_PLOT <- DF_TO_PLOT2 %>% group_by(CAPTURE_KIT_simp) %>%
      wilcox_test(read_depth ~ Race_tissue) %>% add_xy_position(x = 'Race_tissue',scales=c('free'))
    ...
    CAPKITS_cancer_plot <- ggplot(CAPKITS_cancer, aes(x=factor(Race_tissue), y=log(read_depth,base=2),fill=Race_tissue)) + xlab("") + ylab("") + facet_grid(Cancer~CAPTURE_KIT_simp,scales='free',drop=FALSE,labeller = label_wrap_gen(width=20)) + #in 'fill' argument can change factor order
      ...
      {if (CANCER_NAME != 'BRCA'){coord_cartesian(ylim=c(log(30000000,base=2),NA),clip='off')}} + 
      
      
      {if (CANCER_NAME == 'COAD'){coord_cartesian(ylim=c(log(12000000,base=2),NA),clip='off')}} + 
      
      
      {if (CANCER_NAME == 'BRCA'){coord_cartesian(ylim=c(log(30000000,base=2),NA),clip='off')}} + 
      
      {if (CANCER_NAME == 'KIRC'){coord_cartesian(ylim=c(log(45000000,base=2),log(300000000,base=2)),clip='off')}} + 
      ...
      #add text annotation
      {if(CANCER_NAME=='BRCA'){geom_text(data = Counts_CK,aes(label=read_depth, y=23.85,vjust=0),size=12)}} + 
      {if(CANCER_NAME=='COAD'){geom_text(data = Counts_CK,aes(label=read_depth, y=22.40,vjust=0),size=12)}} + 
      
      {if(CANCER_NAME=='PRAD'){geom_text(data = Counts_CK,aes(label=read_depth, y=24.15,vjust=0),size=12)}} + 
      
      
      {if (CANCER_NAME == 'UCEC' | CANCER_NAME == 'LUAD'){geom_text(data = Counts_CK,aes(label=read_depth, y=24.10,vjust=0),size=12)}} + 
      {if (CANCER_NAME == 'KIRC' ){geom_text(data = Counts_CK,aes(label=read_depth, y=24.80,vjust=0),size=12)}}
...
ggarrange(NULL,NULL,NULL,CAPTURE_KIT_MAPPED_READS,nrow=2,ncol=2,common.legend=TRUE,legend='bottom',hjust=00,align='hv',widths=c(0.02,1),heights=c(0.03,1),labels=c('Supplementary Figure 5','','',''),font.label = list(size = 70,color='black')) + theme(plot.margin = margin(b=0.5,l=0.1,t=0.1,r=0.1, "cm")) 
dev.off()
...
NIMBLEGEN_BY_CAPTURE_BRCA_OVERALL_READ_DEPTHS <- function(){
  ...
  STATS_FOR_PLOT <- DF_TO_PLOT %>% group_by(tissue.definition,CAPTURE_KIT_simp) %>%
    wilcox_test(read_depth_per_mb_capkit ~ race_PCA) %>% add_xy_position(x = 'race_PCA',scales=c('free'))
  
  #Modify the Y position manually so that it's same for all facets
  STATS_FOR_PLOT$y.position <- log(max(DF_TO_PLOT$read_depth_per_mb_capkit)*1.25,base=2)
  ...
  DF_TO_PLOT$race_PCA <- factor(DF_TO_PLOT$race_PCA, levels=c('African','European'),ordered=TRUE)
  ...
  plot <- ggplot(DF_TO_PLOT, aes(x=factor(race_PCA),y=read_depth_per_mb_capkit,fill=race_PCA)) + xlab("") + ylab(paste0("Millions of reads per exome per Mb of capture region (log2 scale) ")) + facet_grid(tissue.definition~CAPTURE_KIT_simp,labeller = label_wrap_gen(width=19.5)) + 
    ...
    theme(strip.text.x = element_text(size = 20),#,hjust=0),
          ...
          axis.text.x = element_blank(),
          axis.text.y = element_text(size=20,margin = margin(r=-2)),
    ...
    #add text annotations
    geom_text(data = subset(Counts, tissue.definition == "Primary Tumor"),aes(label=read_depth_per_mb_capkit, y=COORD_BOT,vjust=0.25),size=6) + 
    ...
    geom_text(data = subset(Counts, tissue.definition == "Patient Germline"),aes(label=read_depth_per_mb_capkit, y=COORD_BOT,vjust=0.25),size=6) + 
    ...
    geom_text(data= tumor_label_counts,aes(label=label,y=COORD_BOT,vjust=0.25,hjust=14.225,fontface="bold"),size=6) +
    geom_text(data= germline_label_counts,aes(label=label,y=COORD_BOT,vjust=0.25,hjust=14.225,fontface="bold"),size=6) +
    
    geom_text(data= tumor_label_mean,aes(label=label,y=COORD_TOP,vjust=0.15,hjust=17.25,fontface="bold"),size=6,parse=TRUE) +
    geom_text(data= tumor_label_mean,aes(label=label,y=COORD_TOP,vjust=0.15,hjust=17.28,fontface="bold"),size=6,parse=TRUE) +
    geom_text(data= tumor_label_mean,aes(label=label,y=COORD_TOP,vjust=0.15,hjust=17.31,fontface="bold"),size=6,parse=TRUE) +
    
    
    geom_text(data= germline_label_mean,aes(label=label,y=COORD_TOP,vjust=0.1,hjust=17.25,fontface="bold"),size=6, parse=TRUE) + 
    geom_text(data= germline_label_mean,aes(label=label,y=COORD_TOP,vjust=0.1,hjust=17.28,fontface="bold"),size=6, parse=TRUE) + 
    geom_text(data= germline_label_mean,aes(label=label,y=COORD_TOP,vjust=0.1,hjust=17.31,fontface="bold"),size=6, parse=TRUE) + 
...
#OUTPUT FIGURE 2
pdf(paste0("/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIG2.pdf"),height=11,width=13,onefile = FALSE)
ggarrange(NULL,NULL,NULL,BRCA_NIMBLEGEN_CAPTURE_OVERALL_DEPTHS,nrow=2,ncol=2,common.legend=TRUE,legend='bottom',hjust=00,align='hv',widths=c(0.02,1),heights=c(0.15,1),labels=c('Figure 2','','',''),font.label = list(size = 30,color='black')) + theme(plot.margin = margin(b=0.5,l=0.1,t=0.1,r=0.1, "cm")) 
dev.off()

#proportions test for enrichment of EUR exomes in NimbleGen v2 Exome kit
prop.test(x = c(380, 38), n = c(583, 162))
...
White_MAFs_DPs_at_ALT_positions <- COMBINE_DATA('nimblegen_capture_NOV19','BRCA','BlackWhite_White_DOWNSAMPLED')   #IF DESIRE DOWNSAMPLING:BlackWhite_White_DOWNSAMPLED
...
plot_MAF_by_coverage_level <- function(DF,INCLUDE_EQNS){
  ...
  DF$Race <- factor(DF$Race, levels=c('African','European'),ordered=TRUE)
  ...
  plot <- ggplot(DF, aes(x=as.numeric(as.character(BAF_1kGenomes)), y=as.numeric(as.character(BAF)))) + #facet_grid(rows=vars(Tissue),cols = vars(Race)) + 
    ...
    geom_smooth(data=DF,position=position_jitter(0.000),method='lm', formula= y~x,aes(linetype=Ancestry,color=(fct_reorder(DF$DP_group, DF$variable2, min))),se=FALSE,size=1.5)   + scale_linetype_manual(name="Ancestry",values=c("African"='dashed',"European"='solid')) + 
    ...
    theme(strip.text.x = element_text(size = 30),#,hjust=0),
          ...
          plot.tag.position = c(0.01, 0.98),
          legend.position=c(0.15,0.74),legend.direction='vertical',
...
Thousand_Genomes_Common_capture_MELTED$variable <- factor(Thousand_Genomes_Common_capture_MELTED$variable, levels=c('African','European'))
...
DF_TO_PLOT <- Thousand_Genomes_Common_capture_MELTED
#calculate
STATS_FOR_PLOT <- DF_TO_PLOT %>%  wilcox_test(value ~ variable) 


#adhere to JNCI conventions for p-values and decimals; also, add asterisk for significant
STATS_FOR_PLOT$p <- ifelse(STATS_FOR_PLOT$p < 0.001, "p<0.001*",
                           ifelse(STATS_FOR_PLOT$p < 0.05 & STATS_FOR_PLOT$p >= 0.045,round(STATS_FOR_PLOT$p, digits=3),
                                  ifelse(STATS_FOR_PLOT$p <= 0.99 & STATS_FOR_PLOT$p >= 0.01,round(STATS_FOR_PLOT$p, digits=2),
                                         ifelse(STATS_FOR_PLOT$p <= 0.009 & STATS_FOR_PLOT$p >= 0.001,round(STATS_FOR_PLOT$p, digits=3),round(STATS_FOR_PLOT$p,digits=1)))))
...
THOU_GENOMES_CAPTURE <-ggplot(Thousand_Genomes_Common_capture_MELTED, aes(x=as.factor(variable),y=value*100,fill=variable)) +  
  ...
  theme(axis.text.x = element_blank(),
        axis.text.y = element_text(size=27,margin=margin(r=3.5,l=3.5)),
  ...
  #coord_cartesian(ylim = c(0,1.35))+
  scale_fill_manual(values=c('#EF5350','#42A5F5'),labels=c('African ','European ')) + #MANUALLY SET FILL COLORS) + #MANUALLY SET FILL COLORS 
  
  #means comparisons
  stat_pvalue_manual(STATS_FOR_PLOT,inherit.aes=FALSE,bracket.size = NA,tip.length = 0,label='p',size=11,hide.ns=FALSE,color='black',y.position = 6) +
  ...
  geom_text(data= Mean_TG_label,aes(label=label,y=-1.15,vjust=0,hjust=26.5,fontface="bold"),size=11,parse=TRUE) + 
...
pdf(paste0("/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIG3.pdf"),height=23.5,width=24)
...
ggarrange(NULL,ROW_1,NULL,ROW_3,NULL,widths=c(0.1,0.1,1,0.05),nrow=5,heights=c(0.145,0.45,0.025,0.55,0.01),labels=c('Figure 3'),font.label=list(size=36,color='black'),hjust = 0)
dev.off()


#OUTPUT FIGURE S63 (downsampling)
png(paste0("/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIGS6.png"),height=10,width=11,units='in',res=500)
ROW1 <- ggarrange(NULL,NULL,NULL,widths=c(0.01,1,0.03),nrow=1,labels='Supplementary Figure 6',font.label=list(size=30,color='black'),hjust=0)
ROW2 <- ggarrange(NULL,LINE_PLOT_WITHOUT_EQNS,NULL,widths=c(0.03,1,0.03),nrow=1)
ggarrange(ROW1,ROW2,heights=c(0.12,1),nrow=2)
dev.off()
...
GENERATE_BAC_PLOT <- function(MIN_READS,DATASET,X_POS,Y_POS){
  ...
  BAC_plot <- ggplot(MAF_subset, aes(x=BAC))+ xlab('BAC') + ylab('Density') +  facet_grid(MIN_READS~TYPE) +
    ...
    geom_line(stat='density',size=0.75,alpha=1) + theme(axis.title.y = element_text(size=32)) +
    ...
    geom_text(aes(x=1/MINIMUM_READS,y=AUC_coords[which.min(abs(1/MINIMUM_READS - AUC_coords$x)),]$y,label = paste0(1/MINIMUM_READS), vjust = -1.10,hjust=0.5),color='black',size=7.5) +
    ...
    theme(strip.text = element_text(size = 26),#,hjust=0),
          ...
          axis.text.x = element_text(size=23,margin=margin(t=3.5,l=0,b=0,r=0)),
          axis.text.y = element_text(size=24,margin=margin(t=,l=3.5,b=0,r=3.5)),
          axis.ticks.length=unit(.25, "cm"),
...
png(paste0('/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIGS9.png'),height=24,width=32,res = 600,units = 'in')
BAC_others <- ggarrange(NULL,NULL,NULL,BAC_fig_UCEC,BAC_fig_PRAD,BAC_fig_LUAD,NULL,NULL,NULL,BAC_fig_COAD,BAC_fig_KIRC,nrow=4,ncol=3,heights=c(0.2,1,0.05,1),labels=c('Supplementary Figure 9'),font.label = list(size = 52, color = "black"),hjust=0)
plot(BAC_others)
dev.off()
...
ALT_READS_PLOT <- function(DATASET){
  ...
  PLOT <- ggplot(subset(DATASET,t_alt_count > 0), aes(x=t_alt_count))+ labs(title='Number of reads supporting somatic\nvariant alternative alleles') + xlab('Number of reads') + ylab('Number of somatic variants\n(log2 scale)') +#  facet_wrap(~race) +
    ...
    theme(strip.text = element_text(size = 21,face='bold'),#,hjust=0),
          axis.text.x = element_text(size=22,angle = 45,margin=margin(t=3.5,r=0,b=0,l=0),hjust = 1),
          axis.text.y = element_text(size=22,margin=margin(r=3.5,3.5)),
          axis.ticks.length=unit(.25, "cm"),
          axis.title.y = element_text(size=25,margin=margin(t=0,r=10,b=0,l=0)),
          axis.title.x = element_text(size=25,margin=margin(t=4,r=0,b=0,l=0)),
...
#OUTPUT FIGURE 4
pdf(paste0("/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIG4.pdf"),height=22,width=24)
...
ggarrange(NULL,ROW_1,NULL,ROW_3,NULL,widths=c(1,0.1,1,0.05),nrow=5,heights=c(0.155,0.45,0.015,0.65,0.005),labels=c('Figure 4'),font.label=list(size=36,color='black'),hjust = 0)
dev.off()
...
png(paste0('/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/FIGS8.png'),height=21,width=13,res = 1400,units = 'in')
ggarrange(NULL,NULL,NULL,ggarrange(variant_count_plot_all,NULL,nrow=1,widths=c(1,0.05)),NULL,NULL,NULL,ggarrange(BAC_fig_all,NULL,nrow=1,widths=c(1,0.01)),nrow=4,ncol=2,widths=c(0.10,1),heights=c(0.25,0.75,0.05,1.50),labels=c('Supplementary Figure 8','','A.','','','','B.',''),font.label = list(size = 30,color = 'black'),hjust=0)
dev.off()
...
somatic_with_vcf_and_normal_info_melted$race_PCA <- factor(somatic_with_vcf_and_normal_info_melted$race_PCA,levels=c('African','European'),ordered=TRUE)
...
#create labels for the annotated text below each facet row
TLOD_label_counts <- data.frame(race_PCA='African',variable=c('TLOD'), read_depth=c(14000000),Cancer=c('BRCA','UCEC','PRAD','LUAD','COAD','KIRC'),label=c('n','','','','',''))
TLOD_label_counts$Cancer <- factor(TLOD_label_counts$Cancer, levels=c('BRCA','UCEC','PRAD','LUAD','COAD','KIRC'))
NLOD_label_counts <- data.frame(race_PCA='African',variable=c('NLOD'), read_depth=c(14000000),Cancer=c('BRCA','UCEC','PRAD','LUAD','COAD','KIRC'),label=c('n','','','','',''))
NLOD_label_counts$Cancer <- factor(NLOD_label_counts$Cancer, levels=c('BRCA','UCEC','PRAD','LUAD','COAD','KIRC'))

TLOD_label_mean <- data.frame(race_PCA='African',variable=c('TLOD'), read_depth=c(14000000),Cancer=c('BRCA','UCEC','PRAD','LUAD','COAD','KIRC'),label=c('bar(x)','','','','',''))
TLOD_label_mean$Cancer <- factor(TLOD_label_mean$Cancer, levels=c('BRCA','UCEC','PRAD','LUAD','COAD','KIRC'))
NLOD_label_mean <- data.frame(race_PCA='African',variable=c('NLOD'), read_depth=c(14000000),Cancer=c('BRCA','UCEC','PRAD','LUAD','COAD','KIRC'),label=c('bar(x)','','','','',''))
NLOD_label_mean$Cancer <- factor(NLOD_label_mean$Cancer, levels=c('BRCA','UCEC','PRAD','LUAD','COAD','KIRC'))


#plot
MAF_quality_by_race <- ggplot(somatic_with_vcf_and_normal_info_melted, aes(x=factor(race_PCA),y=as.numeric(value),fill=race_PCA)) + xlab("") + ylab("Log-likelihood ratio per position (log2 scale)") + facet_grid(variable~Cancer, labeller = label_wrap_gen(width=19)) + labs(title='') + 
  ...
  theme(strip.text.x = element_text(size = 24),#,hjust=0),
        strip.text.y = element_text(size = 22),
        axis.text.x = element_blank(),
        axis.text.y = element_text(size=23,margin=margin(r=-3,l=0)),
        axis.title.y = element_text(size=24,margin=margin(t=0,r=10,b=0,l=0)),
        axis.ticks.x = element_blank(), 
        axis.ticks.length=unit(.25, "cm"),
  ...
  scale_y_continuous(trans=pseudo_log_trans(base=2),breaks = 2^seq(1,14, by = 2),
                     expand = expansion(mult = c(0, 0.175)),labels=unit_format(unit = "",scale = 1,accuracy=4,big.mark = ',')) +
  ...
  stat_compare_means(size=8,method='wilcox.test',mapping=aes(label=..p.signif..),
                     ...
                     symnum.args=list(cutpoints = c(0, 0.001, 0.01, 0.05, 1), symbols = c("p<0.001*", "p<0.01*", "p<0.05*", ""))) +
  
  coord_cartesian(ylim=c(0,NA),clip='off') + scale_fill_manual(values=c('#EF5350','#42A5F5'),labels=c('African ','European ')) + #MANUALLY SET FILL COLORS
...
ggarrange(NULL,MAF_quality_by_race,NULL,nrow=3,common.legend = TRUE,legend = 'bottom',align='hv',heights=c(0.06,0.8,0.055),labels=c('Supplementary Figure 7','',''),font.label = list(size = 32, color = "black"),hjust=0) + theme(plot.margin = margin(b=0.5,l=0.1,t=0.1,r=0.1, "cm")) #incr marrgin at bottom
dev.off()
...
FINAL_TABLE <- MAPPED_READS_FOR_PLOTTING[,c('submitter_id_med','Cancer','tissue.definition','bam_full_path','Sequencing_Center','Source_Site','CAPTURE_KIT_simp','race_self_reported','race_PCA','Tumor_Purity','read_depth')]
...
names(FINAL_TABLE) <- c('TCGA Barcode','Cancer Type','Tissue','BAM Filename','Sequencing Center','Collection Site','Exome Capture Kit','Self-Reported Race and Ethnicity','Genomic Ancestry','Tumor Purity','Mapped Read Depth')

write.table(FINAL_TABLE, file='/home/mayo/m187735/s212975.Wickland_Immunomics/GitHub/Racial_Disparities_in_Exome_Read_Depth/MS_plots/Supplementary_Table_1.txt',sep='\t',row.names=FALSE,quote=FALSE)
